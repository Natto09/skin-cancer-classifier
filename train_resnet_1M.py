...
def main():
    ...
    parser.add_argument("--lr_schedule", choices=["steplr", "plateau"], default="plateau",
                         help="'steplr' decays LR every --lr_step_size EPOCHS regardless of how "
                              ...
                              "different sizes -- 1 epoch on 1M is ~13.5x more steps than on "
                              "100K, so a fixed epoch-based schedule decays LR far too slowly "
                              "relative to data seen on the bigger dataset). 'plateau' instead "
                              "watches val macro recall directly and decays LR when it stalls, "
                              "so it adapts automatically regardless of dataset size. Default "
                              "'plateau' fixes this dataset-size mismatch.")
    parser.add_argument("--lr_step_size", type=int, default=5,
                         help="Epochs between LR decays, only used with --lr_schedule steplr")
    parser.add_argument("--best_model_path", default="skin_cancer_best_resnet_1M.pth")
    parser.add_argument("--checkpoint_path", default="train_checkpoint_1M.pth")
