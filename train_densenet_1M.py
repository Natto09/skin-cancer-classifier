...
# Classes where a missed detection (false negative) is the costly kind of
# mistake for a cancer screen -- used below to pick the "best" checkpoint by
# recall on THESE classes specifically, instead of macro recall averaged
# across all 7 classes (which weights df/vasc/nv equally with mel, diluting
...
def main():
    ...
    parser.add_argument("--lr_schedule", choices=["steplr", "plateau"], default="plateau",
                         help="'steplr' decays LR every --lr_step_size EPOCHS regardless of how "
                              "much data that means (bad when comparing datasets of very "
                              "different sizes). 'plateau' watches val macro recall directly "
                              "and decays LR when it stalls, adapting automatically regardless "
                              "of dataset size. Default 'plateau' fixes this dataset-size mismatch.")
    parser.add_argument("--lr_step_size", type=int, default=5,
                         help="Epochs between LR decays, only used with --lr_schedule steplr")
    parser.add_argument("--best_model_path", default="skin_cancer_best_densenet_1M.pth")
    parser.add_argument("--checkpoint_path", default="train_checkpoint_densenet_1M.pth")
    ...
    # --- 1. Transforms ---
    # Even the 1M dataset's 135 baked-in combos/image are still a FIXED,
    # finite set -- the model can still see the exact same pixels epoch after
    # epoch. Live augmentation applies a DIFFERENT random transform each time
    # an image is loaded, so it never sees quite the same pixels twice across
    # epochs. This is genuine regularization on top of whatever's baked in.
    ...
    # --- 3. Model (DenseNet121, last 10 layers unfrozen -- ensemble member 2/3) ---
    model = models.densenet121(weights="DEFAULT")

    # Freeze everything first, then unfreeze the last ~10 layers of the
    # feature extractor (DenseNet's dense blocks are deeply nested, so we
    # unfreeze by parameter position rather than named top-level children).
    ...
    if device.type == "cuda":
        if len(gpu_ids) > 1:
            ...
            print(f"[GPU] Single GPU in use: cuda:{gpu_ids[0]}")
    ...
    if args.lr_schedule == "plateau":
        ...
        print("[LR SCHEDULE] ReduceLROnPlateau on val macro recall (factor=0.1, patience=2 epochs)")
    else:
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size=args.lr_step_size, gamma=0.1
        )
        print(f"[LR SCHEDULE] StepLR every {args.lr_step_size} epochs (gamma=0.1)")
    ...
    if args.resume and os.path.exists(args.checkpoint_path):
        ...
        print(f"[RESUME] Resuming from epoch {start_epoch}, step {resume_step}, "
              f"best_val_cancer_recall={best_val_metric:.4f}, patience_counter={counter}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Per-class recall lets us isolate the classes that actually matter
        # for a cancer screen. Macro recall (all 7 classes weighted equally)
        # is kept for reference/scheduler input, but the metric that decides
        # "is this the best checkpoint" and drives early stopping is now the
        # AVERAGE RECALL OF JUST mel/akiec/bcc -- a model that trades some
        # nv accuracy for meaningfully better mel/akiec/bcc recall should be
        # preferred, and macro recall over all 7 classes was diluting that
        # signal (df/vasc/nv count equally, masking real gains on the classes
        # we actually care about).
        per_class_recall = recall_score(val_all_labels, val_all_preds,
                                         average=None, zero_division=0,
                                         labels=list(range(num_classes)))
        cancer_idxs = [i for i in range(num_classes) if idx_to_class[i] in CANCER_RELATED_CLASSES]
        val_cancer_recall = float(np.mean([per_class_recall[i] for i in cancer_idxs]))
        val_macro_recall = float(np.mean(per_class_recall))

        print(f"Epoch {epoch+1:02d} | Train Loss: {train_loss:.4f} Acc: {train_acc:.2f}% | "
              f"Val Loss: {val_loss:.4f} Acc: {val_acc:.2f}% | "
              f"Macro Recall: {val_macro_recall:.4f} | "
              f"Cancer-class (mel/akiec/bcc) Recall: {val_cancer_recall:.4f}",
              flush=True)

        prev_lr = optimizer.param_groups[0]["lr"]
        if args.lr_schedule == "plateau":
            scheduler.step(val_cancer_recall)
        else:
            scheduler.step()
        ...
        if val_cancer_recall > best_val_metric:
            best_val_metric = val_cancer_recall
            counter = 0
